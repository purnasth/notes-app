...
const Navbar: React.FC<NavbarProps> = ({ value, onChange, onCategoryChange, onSortChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [prevScrollPos, setPrevScrollPos] = useState(0);
  const [visible, setVisible] = useState(true);
  const [user, setUser] = useState<{ username: string; email: string } | null>(null);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [selectedSort, setSelectedSort] = useState<{ sortBy: string; sortOrder: 'asc' | 'desc' }>({
    sortBy: 'created_at',
    sortOrder: 'desc',
  });
        ...
        >
          <div className="space-y-4">
            ...
            <div className="p-2">
              ...
              <ul className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-0 text-dark">
                {[
                  { id: 'sort-alphabet', label: 'Alphabetical Order', value: 'title' },
                  { id: 'sort-created', label: 'Creation Date', value: 'created_at' },
                  { id: 'sort-modified', label: 'Modified Date', value: 'modified_at' },
                ].map((sort) => (
